...
async function processSearch() {
  ...
  if ( inputData.length === 0 ) {
    console.error(
      `Arquivo ${inputFilePath} não encontrado ou vazio!`
    );
    return;
  }

  let processedData = readExcelFile( outputFilePath );
  let processedCNPJs = new Set(
    processedData.map(( row ) => row.cnpj )
  );

  console.log(
    `🔍 Encontrado ${inputData.length}. Processados: ${processedData.length}`
  );
  ...
  for ( const row of inputData ) {
    ...
    const companyName = row.nomeFantasia
    ...
    for ( const query of generateQueries( companyName )) {
      ...
      if ( searchResults.length > 0 ) {
        const filteredResult = filterResults(
          searchResults,
          companyName
        );
        bestLink = filteredResult
          ? filteredResult.link
          : bestLink;
