...
export class GoogleSearchService {
  ...
  private generateSearchQueries(companyName: string): string[] {
    ...
    const normalizedName = companyName.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    ...
    const domainQueries = domainGuesses.map(domain =>
      `site:${domain} ${exactName}`
    );
  ...
  private async executeSearchQuery(
    ...
    try {
      ...
      if (response.data.error) {
        console.error(`API Error: ${response.data.error.code} - ${response.data.error.message}`);
        return [];
      }
      ...
      return response.data.items.filter(item => item.link && item.title) || [];
    } catch (error) {
      if (axios.isAxiosError(error)) {
        ...
        if (this.isRateLimitError(error) && retryCount < MAX_RETRIES) {
          console.log(`Rate limit hit, retrying (${retryCount + 1}/${MAX_RETRIES})...`);
          await this.delayWithJitter(retryCount);
          return this.executeSearchQuery(query, retryCount + 1);
        }
      } else {
        console.error(`Non-Axios error for query "${query}":`, error);
      }
  ...
  private isBlacklistedDomain(link: string): boolean {
    try {
      ...
      const commonPlatforms = ['google.com', 'facebook.com', 'instagram.com',
        'linkedin.com', 'twitter.com', 'youtube.com',
        'support.google', 'maps.google'];

      if (commonPlatforms.some(platform => domain.includes(platform))) {
        return true;
      }
  ...
  private normalizeAndRankResults(results: SearchResult[], companyName: string): SearchResult[] {
    if (results.length === 0) {
      console.warn(`No results to rank for company: ${companyName}`);
      return [];
    }
    ...
    const scoredResults = results
      .map(result => {
        ...
        if (normalizedDomain.includes(normalizedCompanyName)) {
        ...
        if (officialTerms.some(term => result.title.toLowerCase().includes(term))) {
          score += 50; // Higher weight for official indicators
        }
        if (officialTerms.some(term => result.snippet?.toLowerCase().includes(term))) {
          score += 30;
        }

        // Penalize for common non-company domains that might slip through
        if (domain.includes('facebook') || domain.includes('instagram') ||
          domain.includes('linkedin') || domain.includes('twitter')) {
          score -= 100;
        }

        // Direct domain exact match (before www or subdomain)
        const domainGuesses = this.generateDomainGuesses(companyName);
        if (domainGuesses.some(guess => normalizedDomain === guess)) {
    ...
    return scoredResults.map(({ score, ...rest }) => rest); // Remove score from final results
  }
  ...
  private async fallbackDirectDomainCheck(companyName: string): Promise<SearchResult[]> {
    ...
    for (const domain of domainGuesses) {
      ...
      try {
        ...
        if (response.status < 400) { // Domain exists and responds
          results.push({
          ...
          console.log(`Found valid domain through direct check: ${url}`);
          break; // Stop after finding the first valid domain
        }
      } catch (error) {
        // Domain likely doesn't exist or has connection issues, continue to next guess
        continue;
      }
    }
  ...
  public async searchCompany(companyName: string): Promise<SearchResult[]> {
    console.log(`Starting search for company: ${companyName}`);
    ...
    for (const query of queries) {
      ...
      if (validResultCount >= MAX_RESULTS_PER_COMPANY) {
        console.log(`Already found ${validResultCount} valid results, stopping search`);
        break;
      }
      ...
      try {
        ...
        if (filteredItems.length > 0) {
          console.log(`Found ${filteredItems.length} valid results for query: "${query}"`);
          ...
          console.log(`Current unique domain count: ${validResultCount}`);
        }
      } catch (error) {
        console.error(`Search error for ${companyName} with query "${query}":`, error);
      }
    }

    // If no results from Google, try direct domain checks
    if (allResults.length === 0) {
      console.log(`No results from Google search, trying direct domain checks`);
    ...
    if (allResults.length === 0) {
      ...
      console.log(`No results found, providing fallback URL: ${fallbackURL}`);
      allResults.push({
    ...
    const finalResults = rankedResults.slice(0, this.config.maxLinksPerCompany);
